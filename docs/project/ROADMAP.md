...
```mermaid
timeline
    title Clawless Core Security Harness
    section 全ドキュメント完成（Why + What + How 層）
        REQUIREMENTS.md 作成
        THREAT_MODEL.md 作成
        ARCHITECTURE.md 作成
        CLAUDE_MD_SPEC.md 作成
        DETAILED_DESIGN.md 作成
        ADR 設計提案 作成（35 ADR）
    section プロジェクト基盤 ADR 実装完了
        ADR-033 backlog.yaml + sync-project-views 実装
        ADR-031 STG パイプライン実装
        ADR-032 承認レスモード実装
        ADR-034 自律タスクループ実装
