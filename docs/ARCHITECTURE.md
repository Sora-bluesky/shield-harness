...
| --- | ---------------------------- | ----------------------------------------------------------------------------- | ----------------------------------- | ---------------- | ------------------- |
| 1   | clawless-permission.sh       | PreToolUse                                                                    | ツールガバナンス（4 カテゴリ分類）  | ADR-014          | warden + tobari     |
| 2   | clawless-permission-learn.sh | PermissionRequest                                                             | 適応的承認制御                      | ADR-019          | tobari              |
| 3   | clawless-gate.sh             | PreToolUse                                                                    | 破壊的コマンド + フック回避防御     | ADR-027          | guardrails          |
| 4   | clawless-injection-guard.sh  | PreToolUse                                                                    | インジェクション検出（9 カテゴリ）  | ADR-020          | guardrails + tobari |
| 5   | clawless-user-prompt.sh      | UserPromptSubmit                                                              | ユーザー入力インジェクション        | ADR-020          | tobari              |
| 6   | clawless-evidence.sh         | PostToolUse, PostToolUseFailure, ElicitationResult, TeammateIdle, StopFailure | 証跡記録（SHA-256 hash chain）      | ADR-013          | tobari              |
...
| 9   | clawless-circuit-breaker.sh  | Stop                                                                          | リトライ上限管理                    | ADR-016          | tobari              |
| 10  | clawless-task-gate.sh        | TaskCompleted                                                                 | テスト通過ゲート                    | ADR-016          | tobari              |
| 11  | clawless-precompact.sh       | PreCompact                                                                    | コンテキストバックアップ            | ADR-015          | tobari              |
| 12  | clawless-postcompact.sh      | PostCompact                                                                   | 状態復元 + SHA-256 検証             | ADR-015          | tobari              |
| 13  | clawless-instructions.sh     | InstructionsLoaded                                                            | ルールファイル改竄検知              | ADR-015          | tobari              |
| 14  | clawless-session-start.sh    | SessionStart                                                                  | セッション初期化（モジュール分割）  | ADR-028, ADR-029 | tobari              |
| 15  | clawless-session-end.sh      | SessionEnd                                                                    | セッション終了 + 証跡ファイナライズ | ADR-018          | tobari              |
| 16  | clawless-config-guard.sh     | ConfigChange                                                                  | 設定変更監視 + 不正変更ブロック     | ADR-025          | warden              |
| 17  | clawless-subagent.sh         | SubagentStart                                                                 | スコープ + 予算注入                 | ADR-016          | tobari + warden     |
...
| 6. cjk_encoding_attack   | tobari             | high     | CJK 全角文字による命令注入            |
| 7. zero_width_hidden     | tobari             | high     | ゼロ幅文字、ホモグリフ                |
